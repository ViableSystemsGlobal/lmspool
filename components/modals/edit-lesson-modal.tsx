...
export function EditLessonModal({ open, onOpenChange, lesson, onUpdated }: EditLessonModalProps) {
  ...
  useEffect(() => {
    if (lesson && open) {
      const isYouTube = lesson.contentUrl && isYouTubeUrl(lesson.contentUrl)
      setFormData({
        type: (lesson.type as 'text' | 'video' | 'pdf') || 'text',
        title: lesson.title || '',
        contentUrl: lesson.contentUrl || '',
        contentHtml: lesson.contentHtml || '',
        durationMin: lesson.durationMin?.toString() || '',
      })
      // Set video source based on existing content
      if (lesson.type === 'video' && lesson.contentUrl) {
        setVideoSource(isYouTube ? 'url' : 'upload')
      } else {
        setVideoSource('url')
      }
      setVideoFile(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.type === 'text') {
        ...
        lessonData.contentUrl = undefined
        ...
        if (videoSource === 'upload' && videoFile) {
          ...
          if (uploadResponse.ok) {
            ...
            lessonData.contentHtml = undefined
          } else {
      ...
      if (!response.ok) {
        const errorMsg = data.error || 'Failed to update lesson'
              ...
              <div className="space-y-4">
                ...
                {videoSource === 'url' ? (
                  ...
                  <div>
                    ...
                    {formData.contentUrl && !videoPreview && !isYouTubeUrl(formData.contentUrl) && (
